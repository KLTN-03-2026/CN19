...
const CreateEvent = () => {
  const { eventPlatformFee, eventTransactionFee, gasFee, resaleTransactionFee, resalePriceCap, royaltyFee } = useSystemConfig();
  ...
  const { register, control, handleSubmit, watch, setValue, reset, trigger, formState: { errors } } = useForm({
    defaultValues: {
      title: '',
      category_id: '',
      description: '',
  ...
  // Cập nhật giá trị mặc định từ database khi config load xong
  useEffect(() => {
    if (royaltyFee && resalePriceCap) {
      setValue('royalty_fee_percent', royaltyFee);
      setValue('resale_price_limit_percent', 100 + resalePriceCap);
    }
  }, [royaltyFee, resalePriceCap, setValue]);
  ...
  const onSubmit = async (data) => {
    ...
    // Validation giới hạn giá bán lại (Chống đầu cơ) - QUAN TRỌNG
    if (data.allow_resale && data.resale_price_limit_percent > (100 + resalePriceCap)) {
        toast.error(`Giới hạn giá bán lại không được vượt quá ${100 + resalePriceCap}% theo quy định chống đầu cơ của hệ thống.`);
        return;
    }
        ...
        {steps.map((s, i) => (
          <div key={s.id} className="relative z-10 flex flex-col items-center group">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 ${
              step >= s.id ? 'bg-blue-600 text-white shadow-[0_0_20px_rgba(37,99,235,0.4)]' : 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-500'
            }`}>
            ...
            <div className="space-y-6 animate-in slide-in-from-right-8 fade-in duration-500">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2 relative">
                      <label className="text-xs font-bold uppercase  text-gray-700 dark:text-white dark:text-gray-600 dark:text-gray-400">Danh mục <span className="text-red-500">*</span></label>
                      <div className="relative">
                          <button 
                              type="button"
                              onClick={() => setIsCategoryOpen(!isCategoryOpen)}
                              className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-left flex items-center justify-between placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all text-gray-900 dark:text-white"
                          >
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="relative group group-hover:border-blue-600 transition-all">
                          ...
                          <div className="border-2 border-dashed border-gray-200 dark:border-white/20 rounded-2xl p-6 flex flex-col items-center justify-center space-y-3 bg-gray-50/50 dark:bg-white/[0.02] group-hover:bg-blue-600/5 group-hover:border-blue-600 transition-all overflow-hidden relative min-h-[160px]">
                              ...
                              <div className="text-center relative z-10">
                                  ...
                                  <p className="text-[10px] text-gray-600 dark:text-gray-400 font-bold uppercase mt-1 er italic">JPG, PNG, WEBP (Tối đa 5MB)</p>
                              </div>
                              {previewImage && (
                                  <button 
                                      type="button"
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="relative">
                              <input 
                                  type="text"
                                  placeholder="Dán link video (YouTube, MP4 link...)"
                                  value={watch('video_url')}
                                  onChange={(e) => {
                                      setValue('video_url', e.target.value);
                                      setPreviewVideo(e.target.value);
                                  }}
                                  className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-purple-600 transition-all pr-12 text-gray-900 dark:text-white"
                              />
                              {watch('video_url') && (
                                  <button 
                                      type="button"
                          ...
                          <div className="relative group group-hover:border-purple-600 transition-all">
                              ...
                              <div className="border-2 border-dashed border-gray-200 dark:border-white/20 rounded-2xl p-6 flex flex-col items-center justify-center space-y-3 bg-gray-50/50 dark:bg-white/[0.02] group-hover:bg-purple-600/5 group-hover:border-purple-600 transition-all overflow-hidden relative min-h-[160px]">
                                      ...
                                      previewVideo.includes('youtube.com') || previewVideo.includes('youtu.be') ? (
                                          <div className="absolute inset-0 w-full h-full bg-black z-20">
                                              ...
                                              <button 
                                                  type="button"
                                                  onClick={removeVideo}
                                                  className="absolute top-2 right-2 z-30 w-8 h-8 bg-black/50 hover:bg-red-500 text-white rounded-full flex items-center justify-center transition-all backdrop-blur-md"
                                              >
                                                  <Trash2 className="w-4 h-4" />
                                              </button>
                                          </div>
                                  ...
                                  <div className="text-center relative z-10">
                                      <p className="text-xs font-bold text-gray-900 dark:text-white uppercase">Hoặc nhấn để tải tệp video</p>
                                      <p className="text-[10px] text-gray-600 dark:text-gray-400 font-bold uppercase mt-1 er italic">MP4, MOV (Tối đa 100MB)</p>
                                  </div>
                ...
                <div className="space-y-4 pt-6 border-t border-gray-200 dark:border-white/5">
                    <div className="flex items-center justify-between">
                       ...
                       <span className="text-[10px] font-bold text-blue-600 bg-blue-600/10 px-2 py-0.5 rounded-full">Tải lên ít nhất 1 ảnh</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        ...
                        {previewSeatingCharts.length < 10 && (
                            <div className="relative aspect-video md:aspect-square rounded-2xl border-2 border-dashed border-gray-200 dark:border-white/20 hover:border-blue-600 dark:hover:border-blue-600 transition-all bg-gray-50/50 dark:bg-white/[0.02] flex flex-col items-center justify-center space-y-2 cursor-pointer group">
                                <input 
                                    type="file"
                                    multiple
                                    accept="image/*"
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in slide-in-from-right-8 fade-in duration-500">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                      ...
                      <input 
                          ...
                          className={`w-full bg-gray-50 dark:bg-white/5 border ${errors.end_date ? 'border-red-500' : 'border-gray-200 dark:border-white/20'} rounded-xl px-4 py-3 text-sm font-bold placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all text-gray-900 dark:text-white`}
                      />
                  ...
                  <div className="space-y-2">
                      ...
                      <input 
                          ...
                          className={`w-full bg-gray-50 dark:bg-white/5 border ${errors.end_time ? 'border-red-500' : 'border-gray-200 dark:border-white/20'} rounded-xl px-4 py-3 text-sm font-bold placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all text-gray-900 dark:text-white`}
                      />
               ...
               <div className="space-y-2">
                  ...
                  <div className="relative group cursor-pointer" onClick={() => setIsMapModalOpen(true)}>
                      <input 
                          ...
                          readOnly
                          className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all cursor-pointer pr-12 text-gray-900 dark:text-white"
               ...
               <div className="p-6 bg-blue-600/5 rounded-2xl border border-blue-600/10 flex items-start space-x-4">
                  ...
                  <div className="text-xs text-blue-700 dark:text-blue-400 font-medium leading-relaxed">
            ...
            <div className="space-y-8 animate-in slide-in-from-right-8 fade-in duration-500">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                      <h3 className="text-lg font-black tracking-tight text-gray-900 dark:text-white uppercase">Cấu hình hạng vé</h3>
                      <p className="text-[12px] text-gray-700 dark:text-white font-medium ">Tạo ít nhất một hạng vé để mở bán</p>
                  </div>
                  <button 
                      type="button"
                      onClick={() => append({ tier_name: '', price: '', quantity_total: '', section_name: '', benefits: '' })}
                      className="flex items-center bg-blue-600 text-white px-5 py-2 rounded-xl text-xs font-bold hover:brightness-110 transition-all shadow-lg shadow-blue-600/20"
                  >
               ...
               <div className="space-y-4">
                  {fields.map((field, index) => (
                      <div key={field.id} className="p-6 bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/5 rounded-2xl relative group-item transition-all hover:border-blue-600/30">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                              <div className="space-y-1">
                                  <label className="text-[10px] font-black uppercase  text-gray-600 dark:text-gray-400">Tên hạng vé</label>
                                  <input 
                                      {...register(`ticket_tiers.${index}.tier_name`, { required: true })}
                                      className="w-full bg-transparent border-b border-gray-200 dark:border-white/20 py-1 text-sm font-bold placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all text-gray-900 dark:text-white"
                                      placeholder="VD: Early Bird"
                                  />
                              </div>
                              <div className="space-y-1">
                                  <label className="text-[10px] font-black uppercase  text-gray-600 dark:text-gray-400">Giá vé (VNĐ)</label>
                                  <input 
                                      type="number"
                                  ...
                                  <p className="text-[10px] text-blue-600 dark:text-blue-400 font-medium mt-1 leading-relaxed">
                                      * BTC nhận về: {Math.max(0, watch(`ticket_tiers.${index}.price`) * (1 - (eventPlatformFee + eventTransactionFee) / 100) - gasFee).toLocaleString()} VNĐ (Đã trừ: {eventPlatformFee}% phí sàn & {eventTransactionFee}% phí giao dịch & {gasFee.toLocaleString()}đ phí Blockchain/AI)
                                  </p>
                              </div>
                              <div className="space-y-1">
                                  <label className="text-[10px] font-black uppercase  text-gray-600 dark:text-gray-400">Số lượng</label>
                                  <input 
                                      type="number"
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                              <div className="space-y-1">
                                  <label className="text-[10px] font-black uppercase  text-gray-600 dark:text-gray-400">Khu vực / Chỗ ngồi (Tùy chọn)</label>
                                  <input 
                              ...
                              <div className="space-y-1">
                                  <label className="text-[10px] font-black uppercase  text-gray-600 dark:text-gray-400 ">Quyền lợi (Tùy chọn)</label>
                                  <input 
                                      {...register(`ticket_tiers.${index}.benefits`)}
                                      className="w-full bg-transparent border-b border-gray-200 dark:border-white/20 py-1 text-sm font-medium placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all text-gray-900 dark:text-white"
            ...
            <div className="space-y-8 animate-in slide-in-from-right-8 fade-in duration-500">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                      <h4 className="text-xs font-black uppercase  text-blue-600">Quy định giao dịch</h4>
                      
                      <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-white/5">
                          <div className="space-y-1">
                              <p className="text-xs font-bold text-gray-900 dark:text-white uppercase">Cho phép bán lại</p>
                              <p className="text-[10px] text-gray-600 dark:text-gray-400 font-medium">Người dùng có thể bán lại vé trên Marketplace</p>
                          </div>
                      ...
                      <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-white/5">
                          <div className="space-y-1">
                              <p className="text-xs font-bold text-gray-900 dark:text-white uppercase ">Cho phép chuyển nhượng</p>
                              <p className="text-[10px] text-gray-600 dark:text-gray-400 font-medium">Người dùng có thể tặng hoặc gửi vé cho người khác</p>
                          </div>
                  ...
                  <div className="space-y-6">
                      <h4 className="text-xs font-black uppercase  text-blue-600">Phí và Luật Marketplace</h4>
                      
                      {/* Luôn hiển thị Phí bản quyền */}
                      <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase  text-gray-600 dark:text-gray-400 italic">Phí bản quyền (Cố định)</label>
                          <div className="relative">
                              <div className="w-full bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold opacity-70 flex justify-between items-center text-gray-900 dark:text-white">
                          ...
                          <p className="text-[10px] text-blue-600 dark:text-blue-400 font-medium leading-relaxed">
                              * BTC nhận {watch('royalty_fee_percent')}% hoa hồng vĩnh viễn trên phần LỢI NHUẬN của mỗi giao dịch tại Marketplace.
                          </p>
                      </div>
                              ...
                              <div className="relative">
                                  <input 
                                      ...
                                      {...register('resale_price_limit_percent', { 
                                          ...
                                          max: { value: 100 + resalePriceCap, message: `Tối đa ${100 + resalePriceCap}% để tránh đầu cơ` },
                                          min: { value: 100, message: 'Tối thiểu 100% (bằng giá gốc)' },
                                          valueAsNumber: true
                                      })}
                                      className={`w-full bg-gray-50 dark:bg-white/5 border ${errors.resale_price_limit_percent ? 'border-red-500' : 'border-gray-200 dark:border-white/10'} rounded-xl px-4 py-3 text-sm font-bold placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:border-blue-600 transition-all text-gray-900 dark:text-white`}
                                  />
                      ...
                      <div className="p-4 bg-blue-600/5 rounded-xl border border-blue-600/10 space-y-4">
                          ...
                          <div className="space-y-2">
                               <div className="flex justify-between items-center text-[10px]">
                                  <span className="text-gray-700 font-medium dark:text-white">Người mua trả:</span>
                                  <span className="font-bold dark:text-white">Giá niêm yết + Phí sàn ({resaleTransactionFee}%) + Phí Gas ({gasFee.toLocaleString()}đ)</span>
                               </div>
                               <div className="flex justify-between items-center text-[10px]">
                                  <span className="text-gray-700 font-medium dark:text-white">Người bán nhận:</span>
                                  <span className="font-bold text-green-600">Giá bán - {watch('royalty_fee_percent')}% Bản quyền lợi nhuận</span>
                               </div>
                               <div className="flex justify-between items-center text-[10px]">
                                  <span className="text-gray-700 font-medium dark:text-white">BTC nhận (Bản quyền):</span>
                                  <span className="font-bold text-blue-600">+{watch('royalty_fee_percent')}% Lợi nhuận (về ví BTC)</span>
                               </div>
                               <div className="flex justify-between items-center text-[10px]">
                                  <span className="text-gray-700 font-medium dark:text-white">Hệ thống nhận:</span>
                                  <span className="font-bold text-purple-600">Phí sàn ({resaleTransactionFee}%) + Phí Blockchain/AI ({gasFee.toLocaleString()}đ)</span>
                               </div>
                          </div>
                      </div>
                  </div>
