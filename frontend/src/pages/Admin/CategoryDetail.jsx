...
const CategoryDetail = () => {
  ...
  const [cancelReason, setCancelReason] = useState(''); // 'list' or 'grid'
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'active':
        return <span className="px-2 py-0.5 bg-green-500/10 text-green-500 rounded-md text-[8px] font-black uppercase border border-green-500/20">Hoạt động</span>;
      case 'pending':
        return <span className="px-2 py-0.5 bg-yellow-500/10 text-yellow-500 rounded-md text-[8px] font-black uppercase border border-yellow-500/20">Chờ duyệt</span>;
      case 'cancelled':
        return <span className="px-2 py-0.5 bg-red-500/10 text-red-500 rounded-md text-[8px] font-black uppercase border border-red-500/20">Đã hủy</span>;
      case 'hidden':
        return <span className="px-2 py-0.5 bg-gray-500/10 text-gray-600 rounded-md text-[8px] font-black uppercase border border-gray-500/20">Đã ẩn</span>;
      case 'completed':
        return <span className="px-2 py-0.5 bg-purple-500/10 text-purple-500 rounded-md text-[8px] font-black uppercase border border-purple-500/20">Đã kết thúc</span>;
      case 'settled':
        return <span className="px-2 py-0.5 bg-teal-500/10 text-teal-500 rounded-md text-[8px] font-black uppercase border border-teal-500/20">Đã quyết toán</span>;
      default:
        return <span className="px-2 py-0.5 bg-gray-500/10 text-gray-600 rounded-md text-[8px] font-black uppercase border border-white/5">{status === 'draft' ? 'Bản nháp' : status}</span>;
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-gray-600 animate-pulse uppercase text-[10px] font-black tracking-widest">Đang tải dữ liệu...</p>
      </div>
    );
  }
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          >
            <ChevronLeft className="w-5 h-5 text-gray-700 group-hover:text-neon-green" />
          </button>
          <div className="min-w-0">
            <div className="flex items-center space-x-2 text-[10px] font-black uppercase text-gray-600 mb-0.5 tracking-tighter">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-[#111114] border border-gray-200 dark:border-white/5 p-4 rounded-2xl md:rounded-3xl shadow-sm">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-[10px] font-black text-gray-600 uppercase">Tổng sự kiện</p>
        ...
        <div className="bg-white dark:bg-[#111114] border border-gray-200 dark:border-white/5 p-4 rounded-2xl md:rounded-3xl shadow-sm">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-[10px] font-black text-gray-600 uppercase">Cập nhật cuối</p>
        ...
        <div className="bg-white dark:bg-[#111114] border border-gray-200 dark:border-white/5 p-4 rounded-2xl md:rounded-3xl md:col-span-1 overflow-hidden relative group shadow-sm">
          ...
          <div className="relative z-10">
              <p className="text-[10px] font-black text-gray-600 uppercase mb-1">Trạng thái Ảnh</p>
              <div className="flex items-center space-x-2">
                 <div className={`w-2 h-2 rounded-full ${category.image_url ? 'bg-neon-green animate-pulse' : 'bg-gray-600'}`}></div>
      ...
      <div className="bg-white dark:bg-[#111114] border border-gray-200 dark:border-white/5 rounded-2xl md:rounded-3xl overflow-hidden shadow-sm">
        <div className="px-4 py-3 md:px-6 md:py-4 border-b border-gray-200 dark:border-white/5 flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-gray-50/50 dark:bg-white/[0.02]">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3 w-full xl:w-auto">
            ...
            {/* Organizer Filter */}
            <div className="relative w-full sm:w-auto">
              <Users className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-600 pointer-events-none" />
              <select 
            ...
            <div className="flex items-center bg-gray-100 dark:bg-white/5 p-1 rounded-xl border border-gray-200 dark:border-white/10 ml-auto xl:ml-0">
              <button 
                ...
                className={`p-1.5 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white dark:bg-white/10 text-neon-green shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                title="Danh sách"
              >
              ...
              <button 
                ...
                className={`p-1.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-white/10 text-neon-green shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                title="Lưới"
              >
        ...
        {viewMode === 'list' ? (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-white/[0.01] text-left text-[10px] text-gray-700 uppercase font-black tracking-widest border-b border-gray-200 dark:border-white/5">
                ...
                {filteredEvents.length > 0 ? (
                  filteredEvents.map((event) => (
                    <tr key={event.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.015] transition-colors group">
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center space-x-2 text-xs font-bold text-gray-700 dark:text-gray-400 uppercase tracking-tighter">
                      ...
                      <td className="px-6 py-3 text-sm text-gray-700">
                        <div className="text-xs font-black text-gray-700 dark:text-gray-400 tracking-tighter">{format(new Date(event.event_date), 'dd/MM/yyyy')}</div>
                        <div className="text-[9px] font-black flex items-center space-x-1 mt-0.5 text-gray-700 uppercase tracking-widest">
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-12 text-center text-gray-700">
                      ...
                      <p className="text-xs font-black uppercase text-gray-700">Trống</p>
                    </td>
                  </tr>
                )}
              </tbody>
                  ...
                  <div key={event.id} className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/5 rounded-2xl overflow-hidden hover:border-neon-green/30 transition-all group flex flex-col h-full shadow-sm">
                    <div className="relative aspect-video overflow-hidden">
                      {event.image_url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-white/10 text-gray-600">
                    ...
                    <div className="p-3 flex flex-col flex-1">
                      ...
                      <div className="space-y-1.5 mt-auto">
                        <div className="flex items-center space-x-2 text-[10px] font-bold text-gray-700 dark:text-gray-400  tracking-tighter">
                          <Users className="w-3 h-3 text-neon-green opacity-50" />
                          <span className="truncate">{event.organizer?.organization_name || 'N/A'}</span>
                        </div>
                        <div className="flex items-center justify-between text-[10px] font-black text-gray-700 dark:text-gray-500 uppercase tracking-tight">
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        <button 
                          onClick={() => navigate(`/admin/events/${event.id}`)}
                          className="flex-1 py-2 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl text-[10px] font-black uppercase tracking-tight text-gray-700 dark:text-gray-300 hover:bg-neon-green hover:text-black hover:border-neon-green transition-all"
                        >
                          Chi tiết
                        </button>
                        <button 
                          ...
                          className="p-2 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl text-gray-500 hover:text-blue-500 hover:bg-blue-500/10 transition-all"
                          title={event.status === 'hidden' ? "Hiện" : "Ẩn"}
                        >
                          {event.status === 'hidden' ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                        </button>
                        {event.status !== 'cancelled' && (
                          <button 
                            onClick={() => { setCancelEventId(event.id); setShowCancelModal(true); }}
                            disabled={processingId === event.id}
                            className="p-2 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl text-gray-500 hover:text-red-500 hover:bg-red-500/10 transition-all"
                            title="Hủy khẩn cấp"
                          >
          ...
          <div className="bg-white dark:bg-[#111114] border border-gray-200 dark:border-white/10 rounded-[2.5rem] p-8 w-full max-w-md relative animate-in zoom-in-95 duration-300 shadow-2xl">
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600 font-medium">Thao tác này sẽ dừng mọi hoạt động bán vé.</p>
              </div>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-gray-600 uppercase mb-2 ml-1">Lý do hủy (Sẽ gửi cho người mua)</label>
                <textarea 
