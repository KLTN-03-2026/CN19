...
const BlogGrid = ({ blogs, toggleStatus, handleDelete, getTypeLabel }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 animate-in fade-in zoom-in-95 duration-500">
    {blogs.map((blog) => {
      const typeInfo = getTypeLabel(blog.type);
      return (
        <div key={blog.id} className="bg-white dark:bg-[#111114] rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden shadow-sm hover:shadow-md transition-all duration-500 group flex flex-col h-full">
          <div className="relative aspect-[16/10] overflow-hidden">
            ...
            <div className="absolute top-5 right-5">
              <span className={`px-4 py-2 rounded-2xl text-[10px] font-black backdrop-blur-md border border-white/20 shadow-xl ${typeInfo.color}`}>
                {typeInfo.label}
              </span>
            </div>
          </div>
          ...
          <div className="p-3.5 flex-1 flex flex-col gap-3">
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-2 mb-1">
                <div className="w-5 h-5 rounded-lg overflow-hidden border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex flex-shrink-0 items-center justify-center shadow-sm">
                  {blog.author.avatar_url ? (
                    <img src={blog.author.avatar_url} alt="" className="w-full h-full object-cover" />
                  ) : (
                ...
                <span className="text-[10px] font-black text-gray-500 truncate tracking-tight">{blog.author.full_name}</span>
              </div>

              <h3 className="text-sm font-black text-gray-900 dark:text-white group-hover:text-neon-green transition-colors line-clamp-2 tracking-tight leading-snug">
                {blog.title}
              </h3>
              <div className="flex items-center gap-5 text-xs font-black text-gray-600">
                 <div className="flex items-center gap-1.5"><Eye className="w-4 h-4 text-neon-green" /> {blog.views || 0}</div>
                 <div className="flex items-center gap-1.5"><Heart className="w-4 h-4 text-red-500" /> {blog._count.likes}</div>
                 <div className="flex items-center gap-1.5"><MessageSquare className="w-4 h-4 text-blue-500" /> {blog._count.comments}</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-white/5">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => window.open(`/blog/${blog.slug}`, '_blank')} className="p-2 bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-neon-green rounded-lg transition-all"><ArrowUpRight className="w-3.5 h-3.5" /></button>
                <button onClick={() => toggleStatus(blog.id, blog.status)} className="p-2 bg-gray-50 dark:bg-white/5 text-amber-500 hover:bg-amber-500/10 rounded-lg transition-all"><EyeOff className="w-3.5 h-3.5" /></button>
                <button onClick={() => handleDelete(blog.id)} className="p-2 bg-gray-50 dark:bg-white/5 text-gray-400 hover:text-red-500 rounded-lg transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          </div>
        </div>
